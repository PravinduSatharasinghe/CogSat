...
class GeoBeams:
    def __init__(self, turtle_provider=turtle.Turtle, screen_provider=None):
        # Allow injection of Turtle class (Real or Mock)
        self.TurtleClass = turtle_provider 
        
        # Setup screen if passed, otherwise ignore (handled in env)
        if screen_provider and hasattr(screen_provider, 'setup'):
             # We rely on env to setup screen to avoid duplicate windows in GUI mode
             pass
        elif not screen_provider:
    ...
    def create_and_position_beams(self):
        ...
        for coord, color in zip(self.GEO_beam_coordinates, self.freq):
            ...
            t.speed("fastest")
            t.goto(coord[0], coord[1])  # Move the turtle to the specified coordinates
            self.geo_beams.append(t)

    def setup_geo_users(self):
        for x, y in self.geo_user_coordinates:
            user_turtle = self.TurtleClass()
            user_turtle.penup()
