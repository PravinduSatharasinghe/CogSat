...
class BaseTurtle:
    def __init__(self, x, y, color, direction=90, turtle_provider=turtle.Turtle):
        self.TurtleClass = turtle_provider
        self.turtle_obj = self.TurtleClass()
...
class LEOTurtle(BaseTurtle):
    ...
    def __init__(self, x, y, central_color="red", direction=90, turtle_provider=turtle.Turtle):
        # We don't call super init immediately because we need to set provider first, 
        # but BaseTurtle expects it in init.
        super().__init__(x, y, central_color, direction, turtle_provider)
        ...
        for _ in range(6):
            ...
            if color:
                new_turtle = BaseTurtle(x, y, color, turtle_provider=turtle_provider)
    ...
    def get_coordinates(self):
        coordinates = []
        for turtle_obj in self.all_turtles:
            # handle both real turtle (pos()) and mock (position()) or xcor/ycor
            if hasattr(turtle_obj, 'position'):
                 coordinates.append(turtle_obj.position())
            else:
                 coordinates.append((turtle_obj.xcor(), turtle_obj.ycor()))
        return coordinates
