...
class LeoGeoEnv(gymnasium.Env):
    ...
    def __init__(self, env_config=None, render_mode=None):
        ...
        if not hasattr(self, 'spec') or self.spec is None:
            self.spec = gymnasium.envs.registration.EnvSpec("LeoGeoEnv-v3.2")
    ...
    def step(self, actions):
        ...
        for leo_beam in range(7):
            leo_beam_positions.append(np.array(self.leo3.all_turtles[leo_beam].xcor()))
            leo_beam_positions.append(np.array(self.leo3.all_turtles[leo_beam].ycor()))
            leo_beam_positions.append(np.array(self.leo4.all_turtles[leo_beam].xcor()))
            leo_beam_positions.append(np.array(self.leo4.all_turtles[leo_beam].ycor()))
        ...
        for user, geo_beam in self.geo_system.geo_user_beam.items():
            ...
            l1_turtles=self.leo3.all_turtles, l2_turtles=self.leo4.all_turtles, leo_freq_sub=self.leo3.freq_sub, geo_freq_sub=self.geo_system.freq_sub,
        ...
        for user, leo_beam in self.leo_users.leo3_for_user.items():
            ...
            int_satellite_properties=self.leo_users.l4_all_turtles, geo_beams=self.geo_system.geo_beams, \
            leo_freq_sub=self.leo3.freq_sub, geo_freq_sub=self.geo_system.freq_sub)
            leo3_capacity_cal = leo_bandwidth * math.log2(1 + user_leo1_sinr)
            self.leo_user_capacity.append(leo3_capacity_cal)
            self.leo_user_interference.append(user_leo1_interference)

            if leo_beam not in leo1_per_beam_capacity:
                leo1_per_beam_capacity[leo_beam] = [leo3_capacity_cal]
            else:
                leo1_per_beam_capacity[leo_beam].append(leo3_capacity_cal)
        ...
        for user, leo_beam in self.leo_users.leo4_for_user.items():
            ...
            int_satellite_properties=self.leo_users.l3_all_turtles, geo_beams=self.geo_system.geo_beams, \
            leo_freq_sub=self.leo4.freq_sub, geo_freq_sub=self.geo_system.freq_sub)
            leo4_capacity_cal = leo_bandwidth * math.log2(1 + user_leo2_sinr)
            self.leo_user_capacity.append(leo4_capacity_cal)
            self.leo_user_interference.append(user_leo2_interference)

            if leo_beam not in leo2_per_beam_capacity:
                leo2_per_beam_capacity[leo_beam] = [leo4_capacity_cal]
            else:
                leo2_per_beam_capacity[leo_beam].append(leo4_capacity_cal)
