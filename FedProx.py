...
from fl.client_fedprox import FedProxClient, FedProxClientConfig
...
FEDPROX_MU = 0.05
PROX_INTERVAL_TIMESTEPS = 500

OUTPUT_DIR = Path("logs/FedProx")
...
def build_clients(num_clients: int):
    ...
    for client_id in range(num_clients):
        ...
        clients.append(FedProxClient(config))
...
def main():
    ...
    try:
        print("Starting FedProx-A2C training...")
        print(
            f"NUM_CLIENTS={NUM_CLIENTS}, "
            f"CLIENT_FRACTION={CLIENT_FRACTION}, "
            f"NUM_ROUNDS={NUM_ROUNDS}, "
            f"LOCAL_TIMESTEPS={LOCAL_TIMESTEPS}, "
        ...
        for round_idx in range(1, NUM_ROUNDS + 1):
            ...
            round_log["method"] = "FedProx + random selection"
            round_log["fedprox_mu"] = FEDPROX_MU
            round_log["prox_interval_timesteps"] = PROX_INTERVAL_TIMESTEPS
            ...
            if round_idx % 10 == 0:
                server.global_model.save(
                    str(OUTPUT_DIR / f"global_model_round_{round_idx}.zip")
                )
